...
class FofaSpider(object):
    ...
    def __init__(self,Token,Query,StartPage,SpiderNum):
        self.q = quote(Query)
        ...
        if options.source is not None:
            with open(options.source,'r+',encoding='utf-8') as file:
                for value in file.readlines():
                    value = value.strip('\n')
                    try:
                        spider = FofaSpider(token, value, options.startpage, options.spidernum)
                        spider.run()
                    except:
                        pass
        else:
            spider = FofaSpider(token,options.query, options.startpage, options.spidernum)
            spider.run()
